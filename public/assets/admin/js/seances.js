document.addEventListener("DOMContentLoaded", function () {
    ...
    async function loadSavedSeances() {
        try {
            const response = await fetch("/admin/seances/load", {
                method: "GET",
                headers: {
    ...
    function renderSavedSeances(seancesData) {
        ...
        seancesData.forEach((seance) => {
            ...
            const duration =
                parseTimeToMinutes(seance.end_time) - startTimeMinutes;

            const xPosition =
                (startTimeMinutes - BASE_TIME_START) * minutesPerPixel;
    ...
    // Вызываем функцию загрузки сеансов при загрузке страницы
    loadSavedSeances();
    ...
    movies.forEach((movie) => {
        movie.addEventListener("dragstart", function (event) {
            ...
            if (!duration || isNaN(duration)) {
                console.error(
                    "Ошибка: Длительность фильма не указана или не является числом.",
                );
                return;
            }
    ...
    halls.forEach((hall) => {
        ...
        timeline.addEventListener("drop", async function (event) {
            ...
            const duration = parseInt(
                event.dataTransfer.getData("duration"),
                10,
            );
            ...
            const isOverlapping = existingSeances.some((seance) => {
                const seanceLeft = parseFloat(
                    seance.style.left.replace("px", ""),
                );
                const seanceWidth = parseFloat(
                    seance.style.width.replace("px", ""),
                );
                const seanceStartMinutes =
                    BASE_TIME_START + Math.floor(seanceLeft / minutesPerPixel);
                const seanceEndMinutes =
                    seanceStartMinutes +
                    Math.floor(seanceWidth / minutesPerPixel);
    ...
    saveButton.addEventListener("click", async function () {
        ...
        try {
            const response = await fetch("/admin/seances/save", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": csrfToken,
                },
                body: JSON.stringify({ seances: pendingSeances }),
            });

            if (!response.ok) {
                const errorData = await response.json();
                alert(
                    `Ошибка: ${errorData.message || "Не удалось сохранить сеансы."}`,
                );
                return;
            }
    ...
    async function deleteSeance(seanceElement) {
        ...
        try {
            ...
            // Удаляем элемент с таймлайна только после успешного ответа сервера
    ...
    function deletePendingSeance(seanceElement) {
        ...
        pendingSeances = pendingSeances.filter(
            (seance) =>
                seance.movie_id !== movieId || seance.start_time !== startTime,
        );
